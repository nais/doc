...
The database is backed up nightly at 3 AM \(GMT+1\) by default, but can be overridden in [`nais.yaml`](../../workloads/application/reference/application-spec.md#gcpsqlinstancesautobackuptime) by setting `spec.gcp.sqlInstances[].autoBackupTime`.
...
!!! info
    ...
    [Personal database access can also be configured using the nais-cli][nais-cli]
