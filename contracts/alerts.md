...
We use [Prometheus](https://prometheus.io/) to collect metrics, and can trigger alerts based on these metrics. Custom alerts can be specified in the [NAIS manifest](/contracts).


## Custom alerts

Below you can find an example for a custom alert defined in the [NAIS manifest](/contracts):
...
Every app running on NAIS must have a team associated with it. The team needs to be set in the [NAIS manifest](/contracts).
