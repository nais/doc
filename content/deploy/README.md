...
Note: Using _naisd_ or _JIRA Autodeploy_ to deploy your application? These mechanisms are deprecated and are going to be shut down. See [migration guide from naisd to Naiserator](migrating_from_naisd.md). All information on the current page relates to Naiserator compatible `nais.yaml` files. You can also read the [naisd user documentation](naisd.md).
...
* Create a [nais.yaml](nais-manifest.md) file for any application you want to deploy. A `nais.yaml` file contains an _Application resource_. The application resource provides NAIS with the necessary information to run your application. If starting out for the first time, try the [nais.yaml minimal example](https://github.com/nais/doc/tree/c4e8deb972ab60a39a34f32ddc8b7e9b954ec92a/content/deploy/examples/nais-manifest/nais.yaml).
...
See seperate [circleci](circleci.md)-documentation for this.
...
Performing deployments manually requires that you have [access to the cluster](../getting-started/install-tools.md) and `kubectl` configured.
